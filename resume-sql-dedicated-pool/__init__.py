...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if (subscription_id and resource_group_name and workspace_name and sql_pool_name):
        try: 
            ...
            result = mgmt_client.sql_pools.begin_resume(resource_group_name, workspace_name, sql_pool_name)
            return func.HttpResponse(f'Status of resume operation on SQL dedicated pool "{ sql_pool_name}" : "{ result.status() }".')
